...
/// Generous cap for the whole download; a multi-MB release over a slow link must
/// not trip the short timeout used by the update check.
...
fn download_and_stage(version: &str, app: &AppHandle) -> Result<(), String> {
    ...
    let result = (|| -> Result<(), String> {
        ...
        // 3. Extract the exe (+ dll) from the verified zip.
        let extract_dir = temp_dir.join("extracted");
        let (new_exe, new_dll) = extract_target_files(&zip_path, &extract_dir)?;

        // 4. Stage alongside the running exe (copy, since temp may be on another volume).
...
fn apply_and_restart() -> Result<(), String> {
    ...
    // Swap the dll if one was staged.
    let staged_dll = install_dir.join(format!("{DLL_NAME}.new"));
